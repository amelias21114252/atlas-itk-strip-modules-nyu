#!/usr/bin/env python3
#python get_test_timestamp.py --serial_number 20USBHX2002592 --test_name 'Response Curve TC'
...
def format_timestamp(ts):
    """
    Convert:
    2025-04-09T12:34:56.000Z
    →
    2025-04-09 12:34:56.000
    """
    if not ts:
        return None
...
def get_timestamp(client, serial):
    try:
        component = client.get("getComponent", json={"component": serial})
        ...
        test_run = client.get("getTestRun", json={"testRun": test_ids[0]})

        if test_run.get("components"):
            raw_ts = test_run["components"][0].get("stateTs")
...
def main():
    ...
    for i, serial in enumerate(serial_numbers, start=1):
        ...
        ts = get_timestamp(client, serial)
    ...
    print("\n✅ Saved timestamps list to", output_file)
