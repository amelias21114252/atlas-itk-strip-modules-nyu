...
def parse_output_for_errors(output, serial):
    ...
    for block in blocks[1:]:
        ...
        low_match = low_pattern.search(block)

        if low_match:

            low_count = int(low_match.group("count"))

            low_channels = json.loads(low_match.group("channels"))

            low_values = json.loads(low_match.group("values"))
            ...
            print(f"\n🚩 CATEGORY C: {serial} | {filename}")

            print(f"   low_values = {fmt_values(low_values)}")
...
def main():
    ...
    for i, serial in enumerate(serial_numbers, start=1):
        ...
        cmd = [
            ...
            "plot_combined_inputnoise_noskip_categoryBandC.py",
    ...
    print(f"\n🚩 Category C input noise errors < {NOISE_THRESHOLD}:")
    ...
    if category_C_errors:
        ...
        for err in category_C_errors:
            ...
            print(
                ...
                f"low_values = {fmt_values(err['low_values'])}"
    ...
    bad_modules = sorted(set(err["serial"] for err in category_C_errors))

    print(f"\n🚩 Modules with input noise < {NOISE_THRESHOLD}:")
